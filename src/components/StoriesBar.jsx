...
export default function StoriesBar({ onTriggerToast }) {
    ...
    return (
        <div className="flex items-center gap-7 overflow-x-auto pb-4 hide-scrollbar mb-8 pt-2 max-w-[560px] w-full mx-auto self-center">
            {/* Add Story */}
            <div 
                onClick={() => onTriggerToast("Add Story? Your life is already a movie! 🎬", "story")}
            ...
            {mockStories.map((story) => (
                <div 
                    key={story.id} 
                    onClick={() => onTriggerToast(`Watching ${story.name}'s movie... 🍿`, "funny")}
                    className="flex flex-col items-center gap-2 shrink-0 group cursor-pointer"
                >
