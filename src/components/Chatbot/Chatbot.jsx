...
const parseStructuredSteps = (text) => {
    if (!text) return null;
...
const markdownComponents = {
    h2: ({ children }) => <h2 className="text-white font-semibold text-[14px] mb-2">{children}</h2>,
    h3: ({ children }) => <h3 className="text-white font-medium text-[13px] mb-1.5 mt-2">{children}</h3>,
    p: ({ children }) => <p className="mb-2 last:mb-0 text-neutral-200">{children}</p>,
    ul: ({ children }) => <ul className="space-y-2 pl-1 my-2">{children}</ul>,
    ol: ({ children }) => <ol className="space-y-2 pl-1 my-2">{children}</ol>,
    li: ({ children }) => (
        <li className="flex items-start gap-2 text-neutral-200">
            <span className="mt-[7px] h-1.5 w-1.5 rounded-full bg-violet-400 flex-shrink-0" />
            <span className="flex-1">{children}</span>
        </li>
    ),
    strong: ({ children }) => <strong className="text-white font-semibold">{children}</strong>,
    code: ({ inline, children }) => inline ? (
        <code className="px-1.5 py-0.5 rounded bg-white/8 text-violet-200">{children}</code>
    ) : (
        <code className="block overflow-x-auto rounded-2xl bg-[#13131a] border border-white/6 p-3 text-[12px] text-neutral-200">
            {children}
        </code>
    ),
};
